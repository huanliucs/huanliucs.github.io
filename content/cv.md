## Education

**Hangzhou Dianzi University**, PhD Candidate in Cyberspace Security, *Sep. 2024 – Present*
- School of Cyberspace
- Advisor: [Prof. Pengfei Jiao](https://cspjiao.github.io/)

**Hangzhou Dianzi University**, M.S. in Computer Science and Technology, *Sep. 2021 – Jun. 2024*
- School of Computer
- Advisor: [Prof. Pengfei Jiao](https://cspjiao.github.io/)
- Excellent Master's Thesis Award, 2024

**Shihezi University**, B.S. in Computer Science and Technology, *Sep. 2017 – Jun. 2021*
- School of Information Science and Technology

## Research Experience

**Scalable Foundation Models for Dynamic Heterogeneous Graph Representation Learning** *(Jan. 2026 – Present)*
- Initiated research on Dynamic Heterogeneous Graph foundation models, exploring unified tokenization, large-scale self-supervised pretraining, and cross-task transferability.

**Dynamic Heterogeneous Graph Representation Learning Survey** *(Oct. 2025 – Jan. 2026)*
- Established a unified formalism and algorithm-centric taxonomy for Dynamic Heterogeneous Graph representation learning, characterizing temporal modeling biases across embedding-, GNN-, and Transformer-based methods.

**Masked Autoencoder for Dynamic Heterogeneous Graphs** *(Apr. 2025 – Aug. 2025)*
- Proposed a self-supervised framework using a Masked Autoencoder to model spatiotemporal dynamics without manual labels. A hierarchical attention mechanism integrates heterogeneous structure and temporal evolution through reconstruction tasks, achieving 1–4% AUC gains on real-world link prediction.

**High-order Interaction Modeling in Heterogeneous Temporal Graphs** *(Jun. 2024 – Feb. 2025)*
- Designed a heterogeneous temporal hypergraph network (HTHGN) to capture group interactions beyond pairwise relations via P-uniform hyperedges and hierarchical attention, improving representation of dynamic structures.

## Selected Honors

- **Doctoral Student Program of the Young S&T Talents Cultivation Project, CAST** (National-Level) — *Dec. 2025*
- **CSC Joint Doctoral Scholarship Award** (24 months, National-Level) — *May 2025*
- Outstanding Master's Thesis Award, Hangzhou Dianzi University (University-Level) — *Sept. 2025*
- First Grade Academic Scholarship, Hangzhou Dianzi University (Multiple Awarded) — *Oct. 2025*
- Outstanding Graduate, Zhejiang Province & University (Provincial-Level) — *May 2024*
- **National Scholarship for Master's Students** (National-Level) — *Nov. 2023*
- HeyGears Technology Science and Innovation Scholarship (Top 5%) — *Jun. 2023*
- PAT (Programming Ability Test), Advanced Level — *Sept. 2020*
- First Prize, National College Student Mathematical Modeling Competition (Xinjiang Region) — *Sept. 2019*
- First Prize, 10th Lanqiao Cup C/C++ Programming Competition (Xinjiang Region) — *Mar. 2019*
- Honorable Mention, ACM-ICPC Nanchang Invitational — *Jun. 2019*
- Silver Medal, 5th ACM-ICPC Xinjiang Provincial Contest — *May 2019*
- First Prize, 9th ACM-ICPC Shihezi University Invitational — *Apr. 2018*

## Skills

- **Programming:** Python, PyTorch, Deep Graph Library (DGL), PyTorch Geometric (PyG)
- **Languages:** English (CET-4/CET-6), Mandarin (Native Speaker)
